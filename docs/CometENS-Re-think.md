...
## 背景
工作目录：/Users/jason/Dev/aastar/CometENS， 添加子模块：git@github.com:AAStarCommunity/comet-ens-contracts.git，git@github.com:AAStarCommunity/CometENS-frontend.git，git@github.com:AAStarCommunity/CometENS-old.git 在vendor目录（请新建在项目根目录），我们所在分支是aastar-dev分支，以后这个分支是发布分支，因为main是我们fork自其他项目，然后了解三个子模块（是国外的历史版本，和当前项目的思路CometENS），然后了解我的基础思路。
